				...
				<div class="col-sm-12">
					<c:if test='${fn:contains(arrayPermisos, aPermiso )}'>
 						<div class="btn-group pull-right m-t-15">
 							<a href="#" id="addNewCity" data-animation="fadein" data-plugin="custommodal" data-overlaySpeed="200"
 								data-overlayColor="#36404a" class="btn btn-default btn-md waves-effect waves-light m-b-30 w-lg"> 
								<fmt:message key="button.new" />
							</a>
						</div>
					</c:if>
					<h4 class="page-title capitalize"><fmt:message key="text.cities" /></h4>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-12">
								...
								<tr>
									...
									<td>
										<c:forEach var="st" items="${states}">
											<c:if test="${d.estadoid==st.estadoid}">${st.estado}</c:if>
										</c:forEach>
									</td>
									<c:if test='${fn:contains(arrayPermisos, ePermiso )}'>
									<td class="tac">
										<a href="#" id="${d.ciudadid}" data-toggle="modal" data-target="#edit-city-modal" 
									...
									<td class="tac">
										<a href="#" class="table-action-btn" id="${d.ciudadid}" onclick="deleteCity(id);"><i class="md md-close"></i></a>
									</td>
									</c:if>
								</tr>
								</c:forEach>
...
<!-- Modal -->
<div id="city-modal" class="modal-demo">
				...
				<div class="form-group">
					...
					<div class="col-xs-12">
						...
						<div class="form-group inlineflex w100p">
							...
							<ul class="ddListImg noimgOnList" id="state" style="top:40px"></ul>
						</div>
					</div>
				</div>
			</form>
			<button type="button" onclick="return addCity(this);" class="btn btn-default waves-effect waves-light">
				<fmt:message key="button.save" />
			</button>
			<button type="button" onclick="Custombox.close();" id="addCityCancel" class="btn btn-danger waves-effect waves-light m-l-10">
...
<div id="edit-city-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="edit-city-modal" aria-hidden="true" style="display:none;">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title capitalize"><fmt:message key="city.update" /></h4>
				<input type="hidden" id="edCityId">
			</div>
				...
				<form id="formsnedit">
					...
					<div class="col-xs-12">
						...
						<div class="form-group inlineflex w100p">
							<input type="text" class="form-control listfiltersel" placeholder="<fmt:message key="text.select" />" >
							<i class="material-icons iconlistfilter">arrow_drop_down</i>
							<ul class="ddListImg noimgOnList" id="edState" style="top:40px"></ul>
						</div>
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" onclick="return updateCity();" class="btn btn-default waves-effect waves-light">
					<fmt:message key="button.save" />
				</button>
...
<script src="resources/assets/js/cities.js"></script>
