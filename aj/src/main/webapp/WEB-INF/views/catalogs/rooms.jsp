				...
				<div class="col-sm-12">
					<c:if test='${fn:contains(arrayPermisos, aPermiso )}'>
 						<div class="btn-group pull-right m-t-15">
 							<a href="#" id="addNewRoom" data-animation="fadein" data-plugin="custommodal" data-overlaySpeed="200"
 								data-overlayColor="#36404a" class="btn btn-default btn-md waves-effect waves-light m-b-30 w-lg"> 
								<fmt:message key="button.new" />
							</a>
						</div>
					</c:if>
					<h4 class="page-title capitalize"><fmt:message key="text.rooms" /></h4>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-12">
						...
						<table id="datatable-buttons" data-order='[[1,"desc"]]' class="table table-striped table-bordered">
							...
							<tbody>
								...
								<tr>
									...
									<td class="tac">
										<a href="#" id="${d.salaid}" data-toggle="modal" data-target="#edit-room-modal" 
									...
									<td class="tac">
										<a href="#" class="table-action-btn" id="${d.salaid}" onclick="deleteRoom(id);"><i class="md md-close"></i></a>
									</td>
									</c:if>
								</tr>
								</c:forEach>
...
<!-- Modal -->
<div id="room-modal" class="modal-demo">
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="form-group inlineflex w100p">
								<label for="room" class="supLlb"><fmt:message key='text.room' /></label>
								<input type="text" class="form-control c39c" id="room" autocomplete="off">
							</div>
						</div>
					</div>
				</div>
			</form>
			<button type="button" onclick="return addRoom(this);" class="btn btn-default waves-effect waves-light">
				<fmt:message key="button.save" />
			</button>
			<button type="button" onclick="Custombox.close();" id="addRoomCancel" class="btn btn-danger waves-effect waves-light m-l-10">
...
<div id="edit-room-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="edit-room-modal" aria-hidden="true" style="display:none;">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title capitalize"><fmt:message key="room.update" /></h4>
				<input type="hidden" id="edRoomId">
			</div>
			<div class="modal-body">
				<form id="formsnedit">
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="form-group inlineflex w100p">
								<label for="edRoom" class="supLlb"><fmt:message key='text.room' /></label>
								<input type="text" class="form-control c39c" id="edRoom" autocomplete="off">
								<script>
									function fillddEdit(val){
										$.ajax({
											type:"POST",
											url:ctx+"/getCompanies",
											async:false,
											success:function(data){
												var info=data[0],id='redc';
												if(info.length>0){
													for(i=0;i<info.length;i++)
														$('#'+id).append('<li value="'+info[i].companyid+'" title="'+info[i].company+'">'+info[i].company+'</li>');
													$('#'+id).append('<li> </li>');
													getTextDDFilterByVal('redc',val);
												}
											},error:function(e){
												console.log("Error al obtener Firmas. "+e);
											}
										});
									};
								</script>
					...
					<c:if test="${fn:length(comp)>0}">
					<div class="row">
						<div class="col-xs-12">
							<label for="redc" class="supLlb"><fmt:message key='text.firm' /></label>
							<div class="form-group fieldinfo inlineblock w100p">
								...
								<ul class="ddListImg noimgOnList" id="redc"></ul>
							</div>
						</div>
					</div>
					</c:if>
			...
			<div class="modal-footer">
				<button type="button" onclick="return updateRoom();" class="btn btn-default waves-effect waves-light">
					<fmt:message key="button.save" />
				</button>
...
<script src="resources/assets/js/room.js"></script>
