				...
				<div class="col-sm-12">
					<c:if test='${fn:contains(arrayPermisos, aPermiso )}'>
 						<div class="btn-group pull-right m-t-15">
 							<a href="#" id="addNewAction" data-animation="fadein" data-plugin="custommodal" data-overlaySpeed="200"
 								data-overlayColor="#36404a" class="btn btn-default btn-md waves-effect waves-light m-b-30 w-lg"> 
								<fmt:message key="button.new" />
							</a>
						</div>
					</c:if>
					<h4 class="page-title capitalize"><fmt:message key="text.actions" /></h4>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-12">
						...
						<table id="datatable-buttons" data-order='[[1,"desc"]]' class="table table-striped table-bordered">
							...
							<tbody>
								...
								<tr>
									<td>${d.descripcion}</td>
									<td class="tac">
										<c:forEach var="m" items="${matters}">
											<c:if test="${d.materiaid == m.materiaid}">
												${m.materia}
											</c:if>
										</c:forEach>
									</td>
									<td>
										<c:forEach var="w" items="${wayslist}">
											<c:if test="${d.viaid == w.viaid}">
												${w.via}
											</c:if>
										</c:forEach>
									</td>
									...
									<td class="tac">
										<a href="#" id="${d.accionid}" data-toggle="modal" data-target="#edit-action-modal" 
									...
									<td class="tac">
										<a href="#" class="table-action-btn" id="${d.accionid}" onclick="deleteAction(id);"><i class="md md-close"></i></a>
									</td>
									</c:if>
								</tr>
								</c:forEach>
...
<!-- Modal -->
<div id="action-modal" class="modal-demo">
				...
				<div class="form-group">
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="form-group inlineflex w100p">
								<label for="action" class="supLlb"><fmt:message key='text.action' /></label>
								<input type="text" class="form-control c39c" id="action" autocomplete="off">
							</div>
						</div>
					</div>
					<div class="col-xs-12">
					...
					<div class="col-xs-12">
						...
						<div class="form-group inlineflex w100p">
							<input type="text" class="form-control listfiltersel" placeholder="<fmt:message key="text.select" />" >
							<i class="material-icons iconlistfilter">arrow_drop_down</i>
							<ul class="ddListImg noimgOnList" id="way" style="top:40px"></ul>
						</div>
					</div>
				</div>
			</form>
			<button type="button" onclick="return addAction(this);" class="btn btn-default waves-effect waves-light">
				<fmt:message key="button.save" />
			</button>
			<button type="button" onclick="Custombox.close();" id="addActionCancel" class="btn btn-danger waves-effect waves-light m-l-10">
...
<div id="edit-action-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="edit-action-modal" aria-hidden="true" style="display:none;">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title capitalize"><fmt:message key="action.update" /></h4>
				<input type="hidden" id="edActionId">
			</div>
			<div class="modal-body">
				<form id="formsnedit">
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="form-group inlineflex w100p">
								<label for="edAction" class="supLlb"><fmt:message key='text.action' /></label>
								<input type="text" class="form-control c39c" id="edAction" autocomplete="off">
							</div>
						</div>
					</div>
					<div class="col-xs-12">
						<label for="edMatter" class="supLlb"><fmt:message key='text.matter' /></label>
						<div class="form-group inlineflex w100p">
							<input type="text" class="form-control listfiltersel" placeholder="<fmt:message key="text.select" />" >
							<i class="material-icons iconlistfilter">arrow_drop_down</i>
							<ul class="ddListImg noimgOnList" id="edMatter" style="top:40px"></ul>
						</div>
					</div>
					<div class="col-xs-12">
						...
						<div class="form-group inlineflex w100p">
							<input type="text" class="form-control listfiltersel" placeholder="<fmt:message key="text.select" />" >
							<i class="material-icons iconlistfilter">arrow_drop_down</i>
							<ul class="ddListImg noimgOnList" id="edWay" style="top:40px"></ul>
						</div>
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" onclick="return updateAction();" class="btn btn-default waves-effect waves-light">
					<fmt:message key="button.save" />
				</button>
...
<script src="resources/assets/js/ttactions.js"></script>
