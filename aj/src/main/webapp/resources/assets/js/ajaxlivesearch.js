...
(function ($) {
    ...
    $.fn.ajaxlivesearch = function (options) {
         ...
         */
        function search(search_object, form, options, bypass_check_last_value, reset_current_page, customData) {
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.status === 'success') {
                                        ...
                                        var responseResultObj = response.result;
        ...
        this.each(function () {
            ...
            query.on('keyup', function (event) {
                ...
                if ($.trim(query.val()).length && keycode === 13) {
                	...
                	//console.log('type' + $("#"+query_id).attr("data-type"));
                	//var customData = $("#"+query_id).data();
                	var customData = {type: $("#"+query_id).attr("data-type")};
                	search(this, form, ls, false, true, customData);
                }
            });
