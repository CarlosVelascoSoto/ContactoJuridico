;function addCommlabel(e){
	var err='',descr=($('#commlabel').val()).trim();
	if(descr=='')
...
function getDetailsToEdit(id){
	...
	$.ajax({
		type:'POST',
		url:ctx+"/getCommLabelById",
		data:"id="+id,
		async:false,
		success:function(data){
			if(data[0].length==0){
				swal(i18n('msg_warning'),i18n('err_unable_get_commlabel'),"error");
				return;
			}
			var info=data[0][0];
			$('#edCommlabelId').val(info.commlabelid);
			$('#edCommlabel').val(info.commlabelname);
		},error:function(resp){
			swal(i18n('msg_warning'),i18n('err_unable_get_commlabel'),"error");
		}
	});
...
function updateCommlabel(){
	var err='',descr=($('#edCommlabel').val()).trim();
	if(descr=='')
		err=i18n('msg_empty_data');
	if(err!=""){
	...
	var param='id='+$('#edCommlabelId').val()+'&description='+descr;
	$.ajax({
		type:"POST",
		url:ctx+"/updateCommLabel",
		data:param,
		async:false,
		success:function(data){
			if(data=="msg_data_saved"){
...
$("#addNewCommlabel").click(function(){
	$('#errorOnAdd').css('display','none');
	$("#addNewCommlabel").attr('href','#commlabel-modal');
});