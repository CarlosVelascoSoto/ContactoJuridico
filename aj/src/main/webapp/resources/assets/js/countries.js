function addCountry(e){
	var err='',country=($('#country').val()).trim();
	if(country=='')
	...
	var param='pais='+country;
	$.ajax({
		type:"POST",
		url:ctx+"/addNewCountry",
		data:param,
		async:false,
...
function getDetailsToEdit(id){
	...
	$.ajax({
		type:'POST',
		url:ctx+"/getCountryById",
		data:"id="+id,
		async:false,
		success:function(data){
			if(data[0].length==0){
				swal(i18n('msg_warning'),i18n('err_unable_get_country'),"error");
				return;
			}
			var info=data[0][0];
			$('#edCountryId').val(info.paisid);
			$('#edCountry').val(info.pais);
		},error:function(resp){
			swal(i18n('msg_warning'),i18n('err_unable_get_country'),"error");
		}
	});
...
function updateCountry(){
	var err='',country=($('#edCountry').val()).trim();
	if(country=='')
		err=i18n('msg_empty_data');
	if(err!=""){
	...
	var param='paisid='+$('#edCountryId').val()+'&pais='+country;
	$.ajax({
		type:"POST",
		url:ctx+"/updateCountry",
		data:param,
		async:false,
		success:function(data){
			if(data=="msg_data_saved"){
...
$("#addNewCountry").click(function(){
	...
	$("#addNewCountry").attr('href','#country-modal');
});